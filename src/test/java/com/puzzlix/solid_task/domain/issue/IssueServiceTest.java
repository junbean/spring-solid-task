 ...
 * - 단위 테스트
 */
// Mockit 프레임워크를 Junit5와 연동하는 어노테이션
@ExtendWith(MockitoExtension.class) // 가짜 객체
public class IssueServiceTest {
    ...
    /**
     * 테스트 메서드 : 새로운 이슈 생성 시 기능 테스트
     *
     * 테스트 시나리오
     * 1. 클라이언트가 이슈 생성 요청 보냄
     * 2. 서비스는 필요한 연관 엔티티들을 조회함 (User, Project)
     * 3. 새로운 Issue 객체를 생성하고 연관관계를 설정해
     * 4. Issue 엔티티를 저장함
     */
    @Test
    ...
    void createIssue_withMapping() {
        ...
        User mockReporter = new User(1L, "홍길동", "test@test.com", "1234", null);
        Project mockProject = new Project(1L, "솔리드 테스트", "설명설명", null);

        // Mock 객체는 기본적으로 빈 껍데기이다. 실제 동작처럼 시나리오를 미리 작성해 주어야 한다.
        // 실제로 정보가 존재하는지 확인
