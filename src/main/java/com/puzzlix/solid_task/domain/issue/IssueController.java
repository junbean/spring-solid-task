...
public class IssueController {
    ...
    public ResponseEntity<?> deleteIssue(
        ...
        if(userRole != Role.ADMIN){
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body(CommonResponseDto.error("삭제 권한이 없습니다."));
        }
        issueService.deleteIssue(id, userEmail,userRole);

        return ResponseEntity.ok(CommonResponseDto.success(null,"이슈가 성공적으로 삭제되었습니다."));
    }
     ...
     */
    @PostMapping
    public ResponseEntity<CommonResponseDto<Issue>> createIssue(
